...
export async function openRazorpayCheckout(options: RazorpayOptions): Promise<RazorpayResponse | null> {
...
export async function createAndPayOrder(data: {
  tenantId: string;
  planId: string;
  couponCode?: string;
  customerName?: string;
  ...
  try {
    ...
    const paymentResponse = await openRazorpayCheckout({
      ...
      name: 'NexaROS',
      description: `Subscription — ${checkout.planSlug}`,
    ...
    return { success: true, orderId: paymentResponse.razorpay_order_id };
  } catch (error: any) {
