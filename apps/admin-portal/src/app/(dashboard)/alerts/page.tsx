...
export default function AlertsTenantPicker() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Alert Center" description="Select a restaurant to view its AI-detected alerts" />
      <Input
        ...
        {loading ? (
          ...
          tenants.map((t) => (
            <Card key={t.id} className="p-4 hover:border-primary transition-colors cursor-pointer"
              onClick={() => router.push(`/alerts/${t.id}`)}>
              ...
              <Button variant="outline" size="sm" className="mt-3 w-full"
                onClick={(e) => { e.stopPropagation(); router.push(`/alerts/${t.id}`); }}>
                Open Alert Center
              </Button>
            </Card>
          ))
